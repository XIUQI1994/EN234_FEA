...
subroutine el_hybrid_3d_global_basic(lmn, element_identifier, n_nodes, node_property_list, &           ! Input variables
    ...
    ! Local Variables
    integer      :: n_points,kint,P_POINTS,n_nodes_real
  ...
  !  real (prec)  ::  B_red(6,length_dof_array)
   ! real (prec)  :: M(1),DMDXI(1,3)
    ...
    x = reshape(element_coords(1:3*n_nodes_real),(/3,n_nodes_real/))
    ...
    if (n_nodes_real == 4) p_points = 1
    if (n_nodes_real == 10) p_points = 4
    if (n_nodes_real == 8) p_points = 1
    if (n_nodes_real == 20) p_points = 8
...
!    D_DEV(5,5) = 1.d0
!    D_DEV(6,6) = 1.d0
!    D_DEV=D_DEV*E/(1.D0+XNU)
    G=0.D0
    G(1:3,1:3)=1.D0
    D_DEV=D-G*E/3.D0/(1.D0-2*XNU)
  ...
  k_uu=0.d0
    ...
    do kint = 1, n_points
        ...
        B = 0.d0
        B(1,1:3*n_nodes_real-2:3) = dNdx(1:n_nodes_real,1)
        B(2,2:3*n_nodes_real-1:3) = dNdx(1:n_nodes_real,2)
        B(3,3:3*n_nodes_real:3)   = dNdx(1:n_nodes_real,3)
        B(4,1:3*n_nodes_real-2:3) = dNdx(1:n_nodes_real,2)
        B(4,2:3*n_nodes_real-1:3) = dNdx(1:n_nodes_real,1)
        B(5,1:3*n_nodes_real-2:3) = dNdx(1:n_nodes_real,3)
        B(5,3:3*n_nodes_real:3)   = dNdx(1:n_nodes_real,1)
        B(6,2:3*n_nodes_real-1:3) = dNdx(1:n_nodes_real,3)
        B(6,3:3*n_nodes_real:3)   = dNdx(1:n_nodes_real,2)

        K_UU(1:3*n_nodes_real,1:3*n_nodes_real)= K_uu(1:3*n_nodes_real,1:3*n_nodes_real)+&
         matmul(transpose(B(1:6,1:3*n_nodes_real)),matmul(D_DEV,B(1:6,1:3*n_nodes_real)))*w(kint)*determinant
...
!write(*,*) m
        ...
        K_UP(1:3*n_nodes_real,1:P_POINTS)= K_UP(1:3*n_nodes_real,1:P_POINTS)+&
        MATMUL(TRANSPOSE(B(1:6,1:3*n_nodes_real)),BM(1:6,1:P_POINTS))*w(kint)*determinant
        K_PU(1:P_POINTS,1:3*n_nodes_real)= K_pu(1:P_POINTS,1:3*n_nodes_real)+&
        MATMUL(TRANSPOSE(BM(1:6,1:P_POINTS)),B(1:6,1:3*n_nodes_real))*w(kint)*determinant
 ...
 p(1:p_points)=dof_total(3*n_nodes_real+1:3*n_nodes_real+p_points)+&
 dof_increment(3*n_nodes_real+1:3*n_nodes_real+p_points)
    ...
    do kint = 1, n_points
        ...
        strain = matmul(B(1:6,1:3*n_nodes_real),dof_total(1:3*n_nodes_real))
        dstrain = matmul(B(1:6,1:3*n_nodes_real),dof_increment(1:3*n_nodes_real))

        stress = matmul(D_dev,strain+dstrain)
        stress(1)=stress(1)+pint
        stress(2)=stress(2)+pint
        stress(3)=stress(3)+pint
        updated_state_variables((kint-1)*6+1:kint*6)=stress
       ! write(*,*) stress(1),strain(1),pint,lmn,kint
        element_residual(1:3*n_nodes_real) = element_residual(1:3*n_nodes_real) -&
         matmul(transpose(B(1:6,1:3*n_nodes_real)),stress)*w(kint)*determinant

        end do


 element_stiffness(1:3*n_nodes_real,1:3*n_nodes_real)=K_UU(1:3*n_nodes_real,1:3*n_nodes_real)
 element_stiffness(3*n_nodes_real+1:3*n_nodes_real+p_points,1:3*n_nodes_real)=K_PU(1:P_POINTS,1:3*n_nodes_real)
 element_stiffness(1:3*n_nodes_real,3*n_nodes_real+1:3*n_nodes_real+p_points)=K_UP(1:3*n_nodes_real,1:P_POINTS)
 element_stiffness(3*n_nodes_real+1:3*n_nodes_real+p_points,3*n_nodes_real+1:3*n_nodes_real+p_points)=K_PP(1:P_POINTS,1:P_POINTS)

!write(*,*)dof_total+dof_increment
    return
end subroutine el_hybrid_3d_global_basic


!==========================SUBROUTINE el_linelast_3dbasic_dynamic ==============================
subroutine el_hybrid_3d_global_dynamic(lmn, element_identifier, n_nodes, node_property_list, &           ! Input variables
    n_properties, element_properties,element_coords, length_coord_array, &                               ! Input variables
    dof_increment, dof_total, length_dof_array,  &                                                       ! Input variables
    n_state_variables, initial_state_variables, &                                                        ! Input variables
    ...
    return
end subroutine el_hybrid_3d_global_dynamic


!==========================SUBROUTINE fieldvars_linelast_3dbasic ==============================
subroutine fieldvars_hybrid_3d_global_basic(lmn, element_identifier, n_nodes, node_property_list, &           ! Input variables
    n_properties, element_properties,element_coords,length_coord_array, &                                ! Input variables
    dof_increment, dof_total, length_dof_array,  &                                                      ! Input variables
    ...
    real (prec)  ::  smises                          ! Pressure and Mises stress
     ...
     integer      :: n_points,kint,P_POINTS,k,n_nodes_real
  ...
  do kint=1,n_points
       ...
       stress=updated_state_variables((kint-1)*6+1:kint*6)
        ...
        sdev(1)=sdev(1)-sum(stress(1:3))/3
        sdev(2)=sdev(2)-sum(stress(1:3))/3
        sdev(3)=sdev(3)-sum(stress(1:3))/3
    ...
    return
end subroutine fieldvars_hybrid_3d_global_basic
