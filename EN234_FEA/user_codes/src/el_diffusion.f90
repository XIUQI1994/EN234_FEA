...
subroutine el_diffusion_2dbasic(lmn, element_identifier, n_nodes, node_property_list, &           ! Input variables
    ...
    real (prec)  ::  q(6)                         ! Stress vector contains [s11, s22, s12]
    ...
    do kint = 1, n_points
        ...
        D = 0.d0
        D(1,1)=1.d0
        D(1,2)=1.d0-3.d0*((strain(2)+dstrain(2))**2.d0)
        D(2,2)=1.d0/DTIME
        D(5,3)=theta*Dcoef
        D(6,4)=theta*Dcoef
        D(3,5)=-1.d0*kappa
        ...
        q=0.d0
        q(1)=strain(1)+dstrain(1)-(strain(2)+dstrain(2))*((strain(2)+dstrain(2))**2.d0-1.d0)
        q(2)=dstrain(2)/DTIME
        q(3)=-1.d0*kappa*(strain(5)+dstrain(5))
        q(4)=-1.d0*kappa*(strain(6)+dstrain(6))
        q(5)=Dcoef*(strain(3)+theta*dstrain(3))
        q(6)=Dcoef*(strain(4)+theta*dstrain(4))
    ...
    return
end subroutine el_diffusion_2dbasic


!==========================SUBROUTINE el_linelast_2dbasic_dynamic ==============================
subroutine el_diffusion_2dbasic_dynamic(lmn, element_identifier, n_nodes, node_property_list, &           ! Input variables
    n_properties, element_properties,element_coords, length_coord_array, &                               ! Input variables
    dof_increment, dof_total, length_dof_array,  &                                                       ! Input variables
    n_state_variables, initial_state_variables, &                                                        ! Input variables
   ...
   D = 0.d0
    ...
    return
end subroutine el_diffusion_2dbasic_dynamic


!==========================SUBROUTINE fieldvars_linelast_2dbasic ==============================
subroutine fieldvars_diffusion_2dbasic(lmn, element_identifier, n_nodes, node_property_list, &           ! Input variables
    n_properties, element_properties,element_coords,length_coord_array, &                                ! Input variables
    dof_increment, dof_total, length_dof_array,  &                                                      ! Input variables
...
!        s33 = -xnu*stress(1)
    ...
    return
end subroutine fieldvars_diffusion_2dbasic
